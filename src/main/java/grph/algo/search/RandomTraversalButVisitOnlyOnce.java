...
public abstract class RandomTraversalButVisitOnlyOnce implements GraphSearchListener
{
	...
	public RandomTraversalButVisitOnlyOnce(Grph graph, int v, Grph.DIRECTION direction,
			Random prng, int numberOfVisits)
	{
		assert graph != null;
		assert v > 0;
		...
		searchStarted();
		IntSet visited = new SelfAdaptiveIntSet(graph.getNumberOfVertices());
		...
		for (int i = 0;; ++i)
		{
			...
			visited.add(v);
			LucIntSet s = graph.getNeighbours(v, direction);
			LucIntSet neighbors = new LucIntHashSet(s.size());
			neighbors.addAll(s);
