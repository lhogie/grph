...
public class ConsoleStepper extends AbstractStepper
{
	...
	public ConsoleStepper(Grph g, final boolean vertexEvent, final boolean edgeEvent,
			final boolean colorEvent, final boolean labelEvent)
	{
	...
	protected void waitForUserInteraction()
	{
		...
		else if (s.matches("[0-9]+"))
		{
			...
			System.out.println("the stepper will now jump " + getNumberOfStepsInAJump()
					+ " step(s)");
		}
		else if (s.equals("help") || s.equals("h"))
		{
			System.out.println(new String(
					new JavaResource(getClass(), "stepper-help.txt").getByteArray()));
