...
public class biggest_graph_possible extends AbstractGrphScript
{
	...
	protected void declareOptions(Collection<OptionSpecification> optionSpecifications)
	{
		...
		optionSpecifications.add(new OptionSpecification("--degree", "-d", "[0-9]+", "3",
				"Set degree of the desired graph"));
		optionSpecifications.add(new OptionSpecification("--seed", "-s", "[0-9]+",
				String.valueOf(System.currentTimeMillis()), "Set the seed for the PRNG"));
		optionSpecifications.add(new OptionSpecification("--step", null, "[0-9]+",
				"10000",
				"Set the number of new vertices for which a message will be given to the user"));
		optionSpecifications.add(
				new OptionSpecification("--verticesOnly", "-V", null, null, "do not store edges"));
	}
}
