...
public class FourSweepIterativeFringeDiameterAlgorithm extends GrphAlgorithm<Integer>
{
	...
	while (diamInf < diamSup)
	{
	    ...
	    while (iFringe > 0 && 2 * minEcc.bfsResult.distances[minEcc.bfsResult.visitOrder.get(iFringe)] > diamInf)
	    {
		iFringe--;
	    }
	    
//	    System.err.print("At most " + (iLast - iFringe) + " remaining sweeps. diameter is in [" + diamInf + ", "
//		    + diamSup + "]\r");

	    int v = minEcc.bfsResult.visitOrder.get(iLast);
	    minEcc.bfsResult.visitOrder.resize(iLast);
	    ...
	    int distAboveFringe = minEcc.bfsResult.distances[minEcc.bfsResult.visitOrder.get(iLast - 1)];
...
//	System.err.println("\nNumber of sweeps : " + nbSweeps);
	...
	new grph.algo.topology.RandomNewmanWattsStrogatzTopologyGenerator().compute(g, 5, .2);
	...
	void checkSameComponentSize(BFSSweep b)
	{
	    ...
	    {
		throw new IllegalStateException("Component size have changed : " + componentSize + " != "
			+ b.componentSize + ". May be the graph is not symmetric.");
	    }
	}
    }
}
